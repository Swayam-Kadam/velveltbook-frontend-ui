...
import {
  HOME_FILTER_AVAILABILITY,
  HOME_FILTER_DEFAULT_LOCATION,
  HOME_FILTER_LOCATIONS,
  HOME_FILTER_NATIONALITIES,
  HOME_FILTER_PRICE_RANGES,
  HOME_FILTER_SERVICE_CATEGORIES,
} from "@/data/shared/home-filters";

const serviceCategories = [...HOME_FILTER_SERVICE_CATEGORIES];
const priceRanges = [...HOME_FILTER_PRICE_RANGES];
const nationality = [...HOME_FILTER_NATIONALITIES];
const locations = [...HOME_FILTER_LOCATIONS];
const availability = [...HOME_FILTER_AVAILABILITY];
...
function useHomeFilterState() {
  const [selectedLocation, setSelectedLocation] = useState(HOME_FILTER_DEFAULT_LOCATION);
  const [locationInput, setLocationInput] = useState(HOME_FILTER_DEFAULT_LOCATION);
...
type HomeFilterState = ReturnType<typeof useHomeFilterState>;

function HomeFilterFields({
  state,
  onApply,
  idSuffix = "",
}: {
  ...
  const suggestionsId = `location-suggestions${idSuffix}`;
  ...
  }, [locationOpen, selectedLocation, setLocationInput, setLocationOpen]);
    ...
    <div className="space-y-5">
      <section>
        ...
        <div className="relative" ref={locationRef}>
          <div
            className="
              flex h-9 w-full items-center gap-2 rounded-[6px] border
              border-(--border) bg-(--bg-card) px-3 text-[11px]
              text-(--text-primary) focus-within:border-(--accent-primary)
            "
          >
          ...
          {locationOpen && (
            <div
              ...
              role="listbox"
              className="
                absolute z-20 mt-2 w-full overflow-hidden rounded-sm border
                  ...
                  filteredLocations.map((location) => (
                    <button
                      ...
                      className={`
                        ...
                        text-[11px] transition-colors hover:bg-(--bg-secondary)
                        ${
      ...
      <section className="border-t border-(--border) pt-4">
        ...
        <div className="relative mt-3">
          ...
          <button
            ...
            className="
              flex h-8 w-full items-center gap-2 rounded-[6px] border
              border-(--border) bg-(--bg-card) px-3 text-[11px]
              text-(--text-secondary) transition-colors hover:border-(--accent-primary)
            "
          >
            <span
...
export function HomeFilterSidebar({
  ...
  const filterState = useHomeFilterState();
  ...
  return (
    ...
    >
      ...
      >
        ...
        <HomeFilterFields state={filterState} idSuffix="-desktop" />
      </div>
    </aside>
  );
}
